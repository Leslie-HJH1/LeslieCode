...
public class DialogView : MonoBehaviour,IView
{
	...
	private IEnumerator ChangeSize()
	{
		...
		//计算宽度会影响高度  计算高度不会影响宽度
		//所以先计算宽度
		SetWeight(content,frame);
		//当字很多的时候 设置完宽度后 由于LayoutElement是在设置宽度的时候获取的 所以并没有来得及自适应（延时自适应）
		//所以等一帧 等到自适应完成后 再设置高度
		yield return null;
	...
	private void SetWeight(RectTransform content,RectTransform frame)
	{
		...
		else if(weight > _maxWidth)
		{
			...
			content.GetComponent<LayoutElement>().preferredWidth = _maxWidth;//设置成最大宽度
		}
		else
		{
			result = weight;
